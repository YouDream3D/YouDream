...
from .utils.nerf_utils import trunc_exp, MLP, NeRFType, init_decoder_layer
...
class NeRFNetwork(NeRFRenderer):
    def __init__(self, cfg: RenderConfig, num_layers=3, hidden_dim=64, num_layers_bg=2, hidden_dim_bg=64, density_activation='exp'):
        ...
        super().__init__(cfg, latent_mode=cfg.nerf_type == NeRFType.latent, dual_mode=cfg.nerf_type == NeRFType.dual)

        self.num_layers = num_layers
        self.hidden_dim = hidden_dim
        additional_dim_size = 1 if (self.latent_mode or cfg.nerf_type == NeRFType.latent_tune or cfg.nerf_type == NeRFType.dual) else 0
        ...
        self.blob_radius = cfg.blob_radius
        ...
        if cfg.nerf_type == NeRFType.latent_tune or self.dual_mode:
    ...
    def density_blob(self, x, blob_density=10, blob_radius=0.5):
        ...
        if self.density_blob_type == 'none':
            if self.blob_radius == 0:
                return 0.0
            d = (x ** 2).sum(-1)
            g = blob_density * (1 - torch.sqrt(d) / self.blob_radius)
            return g
        d = (x ** 2).sum(-1)
    ...
    def common_forward(self, x, mask=None, **kwargs):
        ...
        if self.decoder_layer is not None and not self.dual_mode:
            albedo = self.decoder_layer(albedo)
            albedo = (albedo + 1) / 2
        elif not self.latent_mode and not self.dual_mode:
            albedo = torch.sigmoid(h[..., 1:])
        elif self.dual_mode:
            albedo_latent_tune = self.decoder_layer(albedo)
            albedo_latent_tune = (albedo_latent_tune + 1) / 2
            return sigma, albedo, albedo_latent_tune
    ...
    def forward(self, x, d, l=None, ratio=1, shading='albedo'):
        ...
        if self.dual_mode:
            sigma, albedo, albedo_latent_tune = self.common_forward(x)
        else:
        ...
        if shading == 'albedo':
            color = albedo
            if self.dual_mode:
                return sigma, color, albedo_latent_tune
            else:
                return sigma, color
        else:
            # normal
            ...
            if self.latent_mode or self.dual_mode:
    ...
    def density(self, x):
        ...
        if not self.dual_mode:
            ...
            'sigma': sigma,
            'albedo': albedo,
            'albedo_latent_tune': albedo_latent_tune,
            }
    ...
    def background(self, d):
        if isinstance(d, str):
            ...
            if self.decoder_layer is not None and not self.dual_mode:
                ...
                rgbs_latent_tune = self.decoder_layer(rgbs)
                rgbs_latent_tune = (rgbs_latent_tune + 1) / 2
                return rgbs, rgbs_latent_tune
