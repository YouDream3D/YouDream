...
class TimePrioritizedScheduler:
    ...
    def get_timestep(self, batch_size, train_step, max_iteration):
        if self.time_sampling == 'uniform':
            ...
            t = torch.ones([batch_size,], dtype=torch.long, device=self.device) * timestep
        elif self.time_sampling == 'annealed_hifa':
            timestep = self.get_annealed_t(train_step, max_iteration)
            t = torch.ones([batch_size,], dtype=torch.long, device=self.device) * timestep
        elif self.time_sampling == 'annealed_random_hifa':
            # timestep = self.get_annealed_t(train_step, max_iteration)
            # t = torch.ones([batch_size,], dtype=torch.long, device=self.device) * timestep
            max_step_i = self.get_annealed_t(train_step, max_iteration)
            t = torch.randint(self.min_step, max_step_i + 1, [batch_size,], dtype=torch.long, device=self.device)
            print(max_step_i)
            print(t)
        else:
    ...
    def get_annealed_t(self, train_step, max_iteration):
        # Current Training Stage
        current_state = train_step / max_iteration
        t = int(self.max_step - (self.max_step - self.min_step) * np.sqrt(current_state))
        # Bi-Search
        # t = self.num_timesteps - bisect.bisect_left(self.weights_cumsum, current_state)  # [0, ..., 1000]
        t = max(t, self.min_step)
        t = min(t, self.max_step)
