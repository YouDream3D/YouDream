...
def rand_pose(size, device, radius_range=(1, 1.5), theta_range=(0, 150), phi_range=(0, 360), view_prompt=None, jitter=False, vertical_jitter=None, batched=False, uniform_sphere_rate=0.5):
    ...
    phis_deg = torch.rad2deg(phis)
    if phis_deg >=180:
        phis_deg = phis_deg - 360

    return poses, dirs, phis_deg
...
def index2pose(index, view_prompt, H=512, W=512, radius=3.0, theta=90, phi=0, eval_size=100, camera_track='circle', device='cuda'):
    # phi = (index / eval_size) * 360
    phi, theta = custom_pose(index / eval_size, eval_theta=theta, fix_camera=False, camera_track=camera_track)
    poses, dirs = circle_pose(device=device, radius=radius, theta=theta, phi=phi, return_dirs=True, view_prompt=view_prompt)
    # fixed focal
    cx = H / 2
    cy = W / 2
    fov = (40 + 70) / 2
    ...
    return intrinsics, poses.cpu().numpy(), dirs
